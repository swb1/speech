...
\subsubsection{Text}
There are two criteria for textual feature selection, which are the augmented tf-idf score and KL distance. We select words with the highest scores as the features based on one of the two criteria.
\begin{itemize}
\item augmented tf-idf: this score is high when the word $w$ occurs many times in a small number of documents. Hence $w$ is highly discriminative in those documents. Denote $d$ as the document and $\mathcal{T}$ as the corpus. We calculate the augmented tf-idf score of a term $t$ for the corpus $\mathcal{T}$ as follows:
\begin{equation}
  tf\textnormal{-} idf(t)^1 =\sum_{d \in \mathcal{T}}\sum_{t \in d} (0.5 + \frac{tf_{t,d}}{\underset{s}{\max}\, tf_{s,d}}) \cdot idf_{t} 
\label{tfidf}
\end{equation}

\item KL distance: this score is high when the word $w$ is most discriminative for class $c$. It is computed as follows: 
\begin{equation}
...
In addition, we have two different approaches for scoring the features in each document $d$, which are the tf-idf and ``tf-kl hybrid'' method.
\begin{itemize}
\item tf-idf: this is the normal tf-idf score for a document $d$.
\begin{equation}
  tf\textnormal{-} idf(t)^2 =tf_{t,d} \cdot idf_{t} 
\label{tfidf1}
\end{equation}
...
We fix the features as openSMILE, and run four classifiers, with PCA whitening on the SVM classifier. We varied different parameters for each classifiers. In particular, for SVM, the set of number of principle components is \{50, 100, 150, 200, 250, 300, 350\}, the gammas is \{0.001, 0.01, 0.1, 1, 10\} and the regularization constant Cs is \{0.001, 0.01, 0.1, 1, 10\}. 
...
If we drop the summation in Equation \ref{kl} and sort its scores for each class $c$, we get the most discriminative words against $c$. Table \ref{genderdis} shows the top discriminative words for gender as an example.
...
In recent speech recognition systems, the word error rate (WER) for the Switchboard corpus roughly ranges from 10\% to 25\% using deep neural network \cite{maas}. We are interested in using the transcribed speech generated from such a system to make prediction on the speakers properties. To simulate a real world speech recognition system, we swap each word in the dataset with a random word in the training set corpus with probability equal to the WER. The set of WERs (\%) we are using is \{0, 10, 15, 20, 25\}.


\subsection{Audio + Text}
For non-categorical variables, i.e., age and education level, we merge the results from audio and text by simply averaging the predictions from both sources on the test set. Averaging makes sense in this case because the labels imply order for non-categorical variables. While this approach slightly improves the age classification accuracy, it doesn't improve or do worse on the education classification and age regression problems.

We also considered various methods such as Platt scaling \cite{platt} and isotonic regression to merge the results for categorical variables, namely, gender and accent. However, we did not implement the solution as it seems neither approach will produce a consistent answer with our SVM predictions. 
